...
- (IBAction)consoleButton:(id)sender {
    
    client::BrowserWindow* bwindow = root_window_->browser_window();
    bwindow->ShowDevTools( root_window_->GetBrowser());
...
void RootWindowMac::CreateRootWindow(const CefBrowserSettings& settings,
  ...
  if (with_controls_) {
    ...
    quit_button_ = MakeButton(&button_rect, @"Quit", contentView);
    [quit_button_ setTarget:delegate];
    [quit_button_ setAction:@selector(quitButton:)];
    [quit_button_ setEnabled:YES];
      
    console_button_ = MakeButton(&button_rect, @"Console", contentView);
    [console_button_ setTarget:delegate];
    [console_button_ setAction:@selector(consoleButton:)];
    [console_button_ setEnabled:YES];
